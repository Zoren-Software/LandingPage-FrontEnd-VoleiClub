...
APP_TITLE="LandingPage Front Setup"
...
update_browserslist_db() {
  if pnpm run | grep -q "browserslist:update"; then
    run_step "Atualizando base do Browserslist (caniuse-lite)" pnpm run browserslist:update
    return 0
  fi

  if pnpm run | grep -q "update-browserslist"; then
    run_step "Atualizando base do Browserslist (caniuse-lite)" pnpm run update-browserslist
    return 0
  fi

  say "ℹ️  Script de update do Browserslist não encontrado no package.json. Pulando etapa."
}
